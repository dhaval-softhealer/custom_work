...
class POSByCategoryWizard(models.TransientModel):
    ...
    sh_session_ids = fields.Many2many('pos.session', string='Session', required=True)
    sh_add_all_session = fields.Boolean(string='Add All Session')
    company_ids = fields.Many2many(
        'res.company', default=lambda self: self.env.companies, string="Companies")

    @api.onchange('sh_add_all_session')
    def _onchange_field(self):
        if(self.sh_add_all_session): 
            print("\n\n\n\n\n\n\n\nsh_add_all_session ====>", self.env['pos.session'].search([]))
            session_ids = self.env['pos.session'].search([])
            self.update({'sh_session_ids':[(6,0,session_ids.ids)] })
        if(not self.sh_add_all_session): 
            self.update({'sh_session_ids':[(6,0,[])] })
    ...
    def print_report(self):
        datas = self.read()[0]
        return self.env.ref('sh_pos_by_category.sh_pos_by_category_action').report_action([], data=datas)


    def display_report(self):
        datas = self.read()[0]
        report = self.env['report.sh_pos_by_category.sh_pos_by_category_doc']
    ...
    def print_xls_report(self):
        ...
        worksheet.write_merge(
            0, 1, 0, 10, 'Report By Point of Sale Category', heading_format)
        ...
        end_date = datetime.strftime(pytz.utc.localize(datetime.strptime(str(self.sh_end_date),
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local), DEFAULT_SERVER_DATETIME_FORMAT)
        worksheet.write_merge(2, 2, 0, 10, start_date + " to " + end_date, bold)
        ...
        if categories:
            for category in categories:
                ...
                if self.sh_session_ids:
                    domain.append(('session_id', 'in', self.sh_session_ids.ids))
                    ...
                    for order in search_orders:
                        if order.lines:
                            ...
                            for line in order.lines.sudo().filtered(lambda x: x.product_id.pos_categ_id.id == category.id):
                                print("\n\n\n\n line -> ", line.product_id.pos_categ_id.name)
                                # self.env['product.product'].sudo().search([('id' ,'=', line.product_id.id)])
                                line_dic = {
                                    ...
                                    # 'unit_price':float("{:.2f}".format(line.price_unit)),
                                ...
                                if line and line.product_id and line.product_id.pos_categ_id and line.product_id.pos_categ_id.name:
                                    line_dic['category'] = line.product_id.pos_categ_id.name,
                                else:
                                    line_dic['category'] = "-",
