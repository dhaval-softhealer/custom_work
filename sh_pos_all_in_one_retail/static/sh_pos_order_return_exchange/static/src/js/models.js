odoo.define("sh_pos_order_list.modelsexchange", function (require) {
    ...
    models.load_models({
        ...
        loaded: function (self) {
            if (self.config.sh_enable_order_list) {
                if (self && self.config && self.config.sh_mode && self.config.sh_mode == "offline_mode") {
                    rpc.query({
                        ...
                        if (orders) {
                            if (orders["order"]) {
                                self.env.pos.db.all_non_return_order = [];
                                self.env.pos.db.all_return_order = [];
                                self.db.add_barcodes(orders["order"]);
        ...
        _save_to_server: function (orders, options) {
            ...
            _.each(orders, function (each_order) {
                ...
                if (each_order["data"]["is_exchange_order"] == true || each_order["data"]["is_return_order"] == true) {
                    ...
                    //     each_order.data['sh_partner_id'] = [each_order.data.partner_id, self.env.pos.db.get_partner_by_id(each_order.data.partner_id).name]
                    // }
                    if (each_order.data && each_order.data.partner_id && self.env.pos.db.get_partner_by_id(each_order.data.partner_id)) {
                        each_order.data['sh_partner_id'] = [each_order.data.partner_id, self.env.pos.db.get_partner_by_id(each_order.data.partner_id).name]
                    }
                    else if (each_order.data && each_order.data.partner_id && self.env.pos.db.get_partner_by_cid(each_order.data.partner_id)) {
                        each_order.data['sh_partner_id'] = [each_order.data.partner_id, self.env.pos.db.get_partner_by_cid(each_order.data.partner_id).name]
                    }
            ...
            return this.rpc(
                            ...
                            if(orders && orders.length > 0){
                                _.each(orders, function (order) {
                                    ...
                                    if(order.is_return_order || order.is_exchange_order){
                                        var return_orders = $.grep(self.db.all_return_order, function (e) {
                                            return e.name != order.pos_reference;
                                        });
                                        self.db.all_return_order = return_orders
                                        self.db.all_return_order.unshift(order)
                                    }else{
                                        var return_orders = $.grep(self.db.all_non_return_order, function (e) {
                                            return e.name != order.pos_reference;
                                        });
                                        self.db.all_non_return_order = return_orders
                                        self.db.all_non_return_order.unshift(order)
                                    }
                                })
                            }
                        })
                ...
                .catch(function (reason) {
                    ...
                    _.each(orders, function (each_order) {
                        ...
                        if (each_order["data"]["is_exchange_order"] == true || each_order["data"]["is_return_order"] == true) {
                            self.db.all_return_order.unshift(each_order.data);
                        } else {
                            self.db.all_non_return_order.unshift(each_order.data);
                        }
