...
def _prepare_data(env, data):
    ...
    if data.get('active_model') == 'sh.label.queue':
        Product = env['sh.label.queue'].with_context(display_default_code=False)
        total = 0
        quantity_by_product = defaultdict(list)
        for p, q in data.get('quantity_by_product').items():
            product = Product.browse(int(p))
            quantity_by_product[product].append((product.product_tmpl_id.barcode, q))
            total += q
...
class ReportPricelistLabel(models.AbstractModel):
    _name = 'report.sh_pricelists_labels.report_productpricelistlabel'
    _description = 'Pricelist Label Report'

    def _get_report_values(self, docids, data):
        return _prepare_data(self.env, data)

class ReportPricelistlDymo(models.AbstractModel):
    _name = 'report.sh_pricelists_labels.report_productpricelistlabel_dymo'
    _description = 'Pricelist Label Report'

    def _get_report_values(self, docids, data):
        return _prepare_data(self.env, data)

class ReportProductPricelistLabel(models.AbstractModel):
    _name = 'report.sh_pricelists_labels.label_product_pricelist_view'
    _description = 'Pricelist Label Report'

    def _get_report_values(self, docids, data):
        if data.get('active_model') == 'sh.label.queue':
            Product = self.env['sh.label.queue']        
