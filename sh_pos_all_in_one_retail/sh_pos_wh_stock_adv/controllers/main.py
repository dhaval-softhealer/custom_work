...
class UpdateRealTimeQuantity(http.Controller):
    ...
    def update_real_time_quantity(self, **post):
        ...
        return [] # Added by Andrew Stratigos, as the below code was throwing an error, this code (I belive) is not being used, and so doing an early escape.
                    # The reason for the error is that the pos session search is not being run as sudo, and is thus returning an error that the publci user (4) cannot execute this method. 
