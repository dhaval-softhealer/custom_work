odoo.define("sh_pos_line_pricelist.models", function (require) {
    ...
    models.load_fields('product.pricelist', ['compute_price_lines', 'compute_label_lines'])

    models.load_models({
        model: "sh.computed.price",
        fields: ['min_quantity', 'pricelist_id', 'product_tmpl_id', 'overridden_price', 'computed_price', 'source'],
        // domain: [['pricelist_id', 'in', self.config.available_pricelist_ids]],
        label: "load computed prices",
        loaded: function (self, all_computed_label) {
            // self.db.all_computed_label = all_computed_label
            var pricelist_by_id = {};
            _.each(self.pricelists, function (pricelist) {
                pricelist_by_id[pricelist.id] = pricelist;
            });
            _.each(all_computed_label, function (line) {
                var pricelist = pricelist_by_id[line.pricelist_id[0]]
                if (pricelist){
                    line.price = line.source == "strategy" ? line.computed_price : line.overridden_price
    ...
    models.Product = models.Product.extend({
        get_price: function(pricelist, quantity, price_extra){
            var self = this;
            var res = _super_Product.get_price.call(this, arguments)
            if (pricelist && pricelist.items){
                _.each(pricelist.items, function (each_data) {
                if ( self.product_tmpl_id === each_data.product_tmpl_id[0]){
                    if (quantity >= each_data.min_quantity){ 
                            res = each_data.price
                        }
                    }
                })
            ...
            if (this.pos.get_order() && this.pos.get_order().get_selected_orderline) {
                ...
                if (self.min_price_pricelist && self.min_price_pricelist.display_price) {
                    if (self.min_price_pricelist.product_tml_id && self.min_price_pricelist.product_tml_id == "All Products" && price < self.min_price_pricelist.display_price && self.is_added) {
                        ...
                        this.price = round_di(parseFloat(parsed_price) || 0, this.pos.dp["Product Price"]);
