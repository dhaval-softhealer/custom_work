odoo.define("sh_pos_product_creation.Product_popup", function (require) {
    ...
    models.load_models({
        label: "Loading Product Category",
        loaded: function (self) {
            if (self.config.create_pos_product) {
            	rpc.query({
                    model: "product.category",
                    method: "search_read",
                }).then(function (product_categorie) {
                	self.env.pos.product_categorie = product_categorie;
                });
            	
            	rpc.query({
                    model: "pos.category",
                    method: "search_read",
                }).then(function (pos_category) {
                	self.env.pos.pos_category = pos_category;
                });
            }
        }
    });
