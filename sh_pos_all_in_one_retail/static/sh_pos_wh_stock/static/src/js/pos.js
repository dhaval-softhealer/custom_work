odoo.define("sh_pos_wh_stock.ProductQtyPopup", function (require) {
            ...
            if (quantity === "remove") {
                ...
                if (unit) {
                    if (unit.rounding) {
                        ...
                        if (round_pr(quant, rounding) && this.pos.config.sh_show_qty_location && !this.pos.config.sh_display_stock_from_other_companies && this.product.type == "product" && !this.product["is_added"]) {
    ...
    class ProductQtyPopup extends AbstractAwaitablePopup {
        ...
        mounted(){
            super.mounted()
            if(this && this.env && this.env.pos && this.env.pos.config && this.env.pos.config.sh_display_stock_from_other_companies && !this.env.pos.config.sh_show_qty_location){
                $('.sh_warehouse_qty_popup').addClass('sh_warehouse_qty_popup_other_company')
            }
        }
    }
    ...
    class QuantityWarningPopup extends AbstractAwaitablePopup {
        ...
        put_order() {
            ...
            if (this.env.pos.config.sh_update_quantity_cart_change && !self.env.pos.is_refund_button_click) {
                if (this && this.env.pos.db && this.env.pos.db.quant_by_product_id && this.product && this.product.id) {
                	var current_quantity = 0
                	if($('.product[data-product-id="' + this.product.id + '"]').find(".sh_warehouse_display") && $('.product[data-product-id="' + this.product.id + '"]').find(".sh_warehouse_display")[0]){
                        current_quantity = $('.product[data-product-id="' + this.product.id + '"]').find(".sh_warehouse_display")[0].innerText 
                    }
                    var actual_quantity = 0.0;
                    actual_quantity = current_quantity - this.product_quantity
                    var list = { product_id: [this.product.id, this.product.display_name], location_id: this.env.pos.config.sh_pos_location, quantity: actual_quantity };
                    $.get(
                        "/update_quanttiy",
                        {
                            passed_list: list,
    ...
    const WHStockProductScreen = (ProductScreen) =>
        class extends ProductScreen {
            ...
            async on_click_show_qty(event) {
                ...
                if(!self.env.pos.config.sh_display_stock_from_other_companies && self.env.pos.config.sh_show_qty_location){
                    let product_id = product.id;
                    let quant_by_product_id = this.env.pos.db.quant_by_product_id[product_id];

                    if (this.env.pos.config.sh_display_by == "location") {
                        var table_html = '<table width="100%" class="wh_qty"><thead><tr><th style="width:40%;">Company</th><th width="40%" class="head_td">Location</th><th width="20%" class="head_td">Quantity</th></tr></thead>';
                        var total_qty = 0;
                        $.each(quant_by_product_id, function (key, value) {
                            ...
                            if (value > 0) {
                                if (self.env.pos.db.lot_stock_list.includes(location["id"])) {
                                    table_html += '<tr><td class="data_td">'+ self.env.pos.company.name +'</td><td class="data_td">' + location["display_name"] + '</td><td class="data_td">' + value + "</td></tr>";
                        ...
                        table_html += '<tr><th width="70%" class="footer_td" colspan="2">Total</th><th width="30%"  class="footer_td">' + total_qty + "</th></tr></table>";
                        ...
                        var table_html = '<table width="100%" class="wh_qty"><thead><tr><th width="40%" class="head_td">Company</th><th width="40%" class="head_td">Warehouse</th><th width="20%" class="head_td">Quantity</th></tr></thead>';
                        var total_qty = 0;
                        $.each(quant_by_product_id, function (key, value) {
                            var warehouse = self.env.pos.db.warehouse_by_id[key];
                            if (warehouse) {
                                total_qty += parseInt(value);
                                table_html += '<tr><td class="data_td">'+ self.env.pos.company.name+'</td><td class="data_td">' + warehouse["name"] + "(" + warehouse["code"] + ')</td><td class="data_td">' + value + "</td></tr>";
                            }
                        });
                        table_html += '<tr><th width="70%" class="footer_td" colspan="2" style="text-align: right;">Total</th><th width="30%"  class="footer_td">' + total_qty + "</th></tr></table>";
                        let { confirmed, payload } = await this.showPopup("ProductQtyPopup", {
                            title: title,
                            body: table_html,
                        });
                ...
                if(self.env.pos.config.sh_display_stock_from_other_companies && !self.env.pos.config.sh_show_qty_location){
                    await this.rpc({
                        ...
                        method: 'sh_get_product_info_pos',
                        args: [[product.id],
                            this.env.pos.config.id],
                        kwargs: {context: this.env.pos_session},
                    }).then(async function(warehouse_detail){
                        let { confirmed, payload } = await self.showPopup("ProductQtyPopup", {
                            title: title,
        ...
        class extends ProductsWidget {
            get productsToDisplay() {
                ...
                if (this.searchWord !== "") {
                    ...
                    if (products.length > 0 && self.env.pos.config.sh_enable_product_suggestion) {
                    ...
                    var product_list = [];
                    if (self.env.pos.is_bag_popup && self.env.pos.config.sh_carry_bag_category && self.env.pos.config.sh_carry_bag_category[0]) {
                        _.each(self.env.pos.db.get_product_by_category(self.env.pos.config.sh_carry_bag_category[0]), function (product) {
                            var quant_by_product_id = self.env.pos.db.quant_by_product_id[product.id];
                            if (self.env.pos.config.sh_show_qty_location && !self.env.pos.config.sh_display_stock_from_other_companies) {
...
odoo.define("sh_pos_wh_stock.screens", function (require) {
    ...
    models.load_models({
        ...
        loaded: function (self) {
            if (self.config.sh_display_stock) {
            	rpc.query({
                    model: "stock.quant",
                    method: "search_read",
                ...
                }).then(function (qunats) {
                	self.env.pos.db.add_qunats(qunats);
                });
            	...
            	rpc.query({
                	...
                	self.env.pos.db.add_warehouse(warehouses);
                });
            	...
            	rpc.query({
                	...
                	self.env.pos.db.add_location(locations);
                });
            	...
            	rpc.query({
                	...
                	self.env.pos.db.add_picking_types(picking_types);
                });
            }
        }
    });
