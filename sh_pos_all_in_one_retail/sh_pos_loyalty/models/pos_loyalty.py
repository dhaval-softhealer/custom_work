...
class ShPosCoupon(models.Model):
    ...
    sh_coupon_value_type = fields.Selection([('fixed', 'Fixed'),
                                             ('percentage', 'Percentage')], default='fixed', required='1', string='Coupon Value Type')
...
class ShPosLoyaltyRule(models.Model):
    ...
    sh_rule_type = fields.Selection([('category', 'Category'),
                                    ...
                                    default='category', string='Rule Type', required=1)
    sh_point_per_product = fields.Float(string='Point Per Product')
    ...
    sh_product_ids = fields.Many2many('product.product',
                                      string='Product')
    sh_brands = fields.Many2many('product.brand', 'sh_product_brands', string="Brands")
    sh_loyalty_earned_rule = fields.Selection([('per_amount_spent','Amount Spent'),('per_product','Product')],string="Points Per",default="per_amount_spent",required="1")
    sh_amount_spent = fields.Float(string="Amount Spent")
    sh_amount_per_point = fields.Float(string="Points Per Amount Spent")
...
class ShPosLoyaltyReward(models.Model):
    ...
    sh_reward_type = fields.Selection([('gift', 'Gift'), ('discount',
                                                          'Discount')], string="Reward Type", default='gift', required=1)
