odoo.define('sh_pos_product_variant.pos', function (require) {
    ...
    models.load_models({
        label: "Loading Product Attribute",
        loaded: function (self) {
            if (self.config.sh_pos_enable_product_variants) {
            	rpc.query({
                    model: "product.template.attribute.line",
                    method: "search_read",
                }).then(function (all_attributes) {
                	self.env.pos.db.product_temlate_attribute_lineids = all_attributes
                    if (all_attributes && all_attributes.length > 0) {
                        _.each(all_attributes, function (each_qr_element) {
                            self.env.pos.db.product_temlate_attribute_line_by_id[each_qr_element.id] = each_qr_element
                        });
                    }
                });
            	...
            	rpc.query({
                	...
                	self.env.pos.db.product_temlate_attribute_ids = all_attributes
                    if (all_attributes && all_attributes.length > 0) {
                        _.each(all_attributes, function (each_qr_element) {
                            self.env.pos.db.product_temlate_attribute_by_id[each_qr_element.id] = each_qr_element
                        });
                    }
                });
            }
